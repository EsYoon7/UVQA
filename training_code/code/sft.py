...
from models.trainer.sft_trainer import *
...
# for debug turn off wandb # added by esyoon 2024-09-14-15:20:53
os.environ['WANDB_MODE'] = 'disabled'
os.environ['WANDB_PROJECT'] = 'UVQA'
...
class TrainingArguments(transformers.Seq2SeqTrainingArguments):
    ...
    lora_enable: bool = field(
        default=False, metadata={"help": "Enable LoRA"}
    )
...
def train():
    ...
    if training_args.lora_enable:
        if len(training_args.lora_modules) == 1:
            training_args.lora_modules = training_args.lora_modules.split(",")
        peft_config = LoraConfig(
            lora_alpha = training_args.lora_alpha,
            lora_dropout = training_args.lora_dropout,
            r = training_args.lora_r,
            target_modules=training_args.lora_modules,
            task_type="CAUSAL_LM",
        )
    else:
        peft_config = None
    ...
    if args.do_train:
        ...
        if args.running_model_name == "video_chat2":
            ...
            for i in range(min(3, len(training_data))):
                ...
                rank0_print("=" * 20)
                ex_input_ids_1 = training_data[i]["input_ids"]
